...
if exist('dataML', 'var') || exist('HHData','var')
    ...
    if Kmeans || allBasicClassifiers
        ...
        if ~bspline
            ...
            % To use bspline information as base
            sp = spmak(1:10,3:8);
            for trials = 1:size(dataML.Data,4)
                for frequencies = 1:size(dataML.Data,1)
                    temp = squeeze(dataML.Data(frequencies,:,:,trials))';
        ...
        if PCA
            ...
            for trials = 1:size(dataML.Data,1)
                ...
                if normBetweenOneAndZeroBeforePCA
            ...
            for coeffs_to_retain = 1:size(scoreMCA,3) %1:size(scoreMCA,3)
            ...
            plotMCCvsFeatures(results.MCC.MCA,1:size(scoreMCA,3),norm(iter),fullfile(resultsDir,'FeaturesvsMCC'),'MCA');
            ...
            plotMCCvsFeatures(results.MCC.CA1.(pcaName),1:size(scoreCA1,3),norm(iter),fullfile(resultsDir,'FeaturesvsMCC'),'CA1');
            ...
            plotMCCvsFeatures(results.MCC.CA3.(pcaName),1:size(scoreCA3,3),norm(iter),fullfile(resultsDir,'FeaturesvsMCC'),'CA3');
