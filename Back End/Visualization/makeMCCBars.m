...
if nargin > 7
    ...
    [index4,~] = find(MCC_Reconstruction);
    % Construct Bar Comparisons
 
    loc = 2:(length(mean(squeeze(MCC_Categories(index3,:,:)),2)) + 1);
    constructBarComparisons = nan(length(loc) + length(index2), 2)
    addC = [MCC_Reconstruction(index) mean(MCC(index2,:),2)];
    constructBarComparisons(1,1:size(addC,2)) = addC;
    addM =  [NaN mean(MCC(index2,:),2)];
    addS = [NaN std(MCC(index2,:),0,2)];
    constructErrorBarPos(1,1:length(addM)) = addM;
    constructErrorBarSTD(1,1:length(addS)) = addS;
    for row = loc-1
        addC = [MCC_Reconstruction(index4) mean(squeeze(MCC_Categories(index3,row,:)))];
    constructBarComparisons(row+1,1:length(addC)) = addC;
    addM = [ NaN mean(squeeze(MCC_Categories(index3,row,:)))];
    addS = [ NaN std(squeeze(MCC_Categories(index3,row,:)))];
...
legendLabels = {'From Reconstructed Clusters','From Multiple Iterations'};
legend(legendLabels);
else
    [index2,~] = find(MCC);
    index2 = unique(index2);
