...
sChannels = [1:10, 17:26, 33:42];
CA1_Channels = [7:10, 23:26, 39:42];
CA3_Channels = [1:6, 17:22, 33:38];
...
timeBin = .3; % 1/16;  % Time Bin Width (s)
...
[minValue,closestIndex] = min(abs(parameters.Derived.freq-gamma(ii)'));
if parameters.Derived.freq(closestIndex) > gamma(ii) && ii == 1
parameters.SpectralAnalysis.Gamma(ii) = parameters.Derived.freq(closestIndex-1);
elseif parameters.Derived.freq(closestIndex) < gamma(ii) && ii == 2
parameters.SpectralAnalysis.Gamma(ii) = parameters.Derived.freq(closestIndex+1);
else
    parameters.SpectralAnalysis.Gamma(ii) = parameters.Derived.freq(closestIndex);
end
end
